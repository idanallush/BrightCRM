      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-tint-lavender text-base font-semibold text-primary">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-caption text-slate">{email}</span>
              {member?.role && <span className="text-caption text-stone">{member.role}</span>}
            </div>
          </div>
          <div className="mt-4 flex items-center gap-3 rounded-lg bg-surface-soft p-3">
            <Send className="h-5 w-5 text-primary" />
            <div className="flex-1">
              <span className="text-sm font-medium text-ink">טלגרם</span>
              <span className="mr-2 text-caption text-slate">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="mt-6 rounded-lg border border-hairline bg-surface-soft p-4">
            ...
            <ul className="flex flex-col gap-2 text-caption text-slate">
              <li className="flex items-start gap-2">
                <MessageCircle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-stone" />
                <span>&quot;פוטוטבע - להכין באנרים לקמפיין חדש עד יום חמישי&quot;</span>
              </li>
              <li className="flex items-start gap-2">
                <Mic className="mt-0.5 h-3.5 w-3.5 shrink-0 text-stone" />
                <span>שלח הקלטה קולית - ה-AI מתמלל ומזהה את הלקוח אוטומטית</span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-stone" />
          ...
          <div className="flex flex-col divide-y divide-hairline-soft">
            {(allMembers ?? []).map((m: any) => (
              <div key={m.id} className="flex items-center gap-3 py-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-tint-lavender text-[11px] font-semibold text-primary">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {m.role && <span className="text-caption text-stone">{m.role}</span>}
                  </div>
                  <span className="text-caption text-slate">{m.email}</span>
                </div>
                <div className="flex items-center gap-2">
                  {m.telegram_user_id ? (
...
function Step({ num, icon, title, desc }: {
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-tint-lavender text-caption font-semibold text-primary">
        {num}
      </div>
      <div className="flex-1">
        <div className="flex items-center gap-2">
        ...
        <p className="mt-0.5 text-caption text-slate">{desc}</p>
      </div>
    </div>
  );
}
