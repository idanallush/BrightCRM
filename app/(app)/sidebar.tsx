    ...
    const link = (
      <Link href={href} aria-current={active ? "page" : undefined}
        className={cn("group relative flex items-center gap-3 rounded-md px-3 py-2.5 text-body-sm transition-colors duration-150",
          active ? "bg-[#CCE5FF] font-semibold text-primary" : "text-ink-secondary hover:bg-surface hover:text-ink",
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-16 shrink-0 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-1">
          ...
          {!isMobile && <button type="button" onClick={() => setCollapsed((v) => !v)} className="rounded-md p-1.5 text-ink-muted transition-colors duration-150 hover:bg-surface hover:text-ink">
            {collapsed ? <ChevronsRight className="h-4 w-4" /> : <ChevronsLeft className="h-4 w-4" />}
          </button>}
          {isMobile && <button type="button" onClick={() => setMobileOpen(false)} className="rounded-md p-1.5 text-ink-muted"><X className="h-5 w-5" /></button>}
        </div>
      </div>
      ...
      <div className="shrink-0 border-t border-border p-3">
        <div className={cn("flex items-center gap-3 rounded-md px-2 py-2", collapsed && !isMobile && "justify-center px-0")}>
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#CCEAFF] text-caption font-semibold text-primary">{initials}</div>
          {(isMobile || !collapsed) && <div className="flex min-w-0 flex-1 items-center justify-between">
            ...
            <button type="button" onClick={signOut} className="rounded-md p-1.5 text-ink-muted transition-colors duration-150 hover:bg-surface hover:text-ink" title="התנתקות"><LogOut className="h-4 w-4" /></button>
          </div>}
        </div>
      </div>
    </div>
