...
import { LayoutDashboard, CheckSquare, Users, Settings, Info, ChevronsLeft, ChevronsRight, X, LogOut, BarChart3 } from "lucide-react";
...
const NAV_MAIN = [
  { href: "/briefing", label: "סקירת בוקר", Icon: BarChart3 },
...
export function Sidebar({ userLabel }: { userLabel: string }) {
  ...
  function NavLink({ href, label, Icon, isMobile, index }: { href: string; label: string; Icon: React.ComponentType<{ className?: string }>; isMobile: boolean; index: number }) {
    ...
    const link = (
      <Link href={href} aria-current={active ? "page" : undefined}
        className={cn("group relative flex items-center gap-3 rounded-md px-3 py-2.5 text-body-sm transition-colors duration-150",
          active
            ? "bg-white/10 font-semibold text-white"
            : "text-white/60 hover:bg-white/5 hover:text-white/90",
          !isMobile && collapsed && "justify-center px-0",
        )} title={label}>
        {active && <span className="absolute right-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-l-full bg-primary" />}
    ...
    <div className="flex h-full flex-col">
      <div className="flex h-16 shrink-0 items-center justify-between px-4">
        <Link href="/dashboard" className="flex items-center gap-2.5">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary text-body-sm font-bold text-white">B</span>
        ...
        <div className="flex items-center gap-1">
          ...
          {!isMobile && <button type="button" onClick={() => setCollapsed((v) => !v)} className="rounded-md p-1.5 text-white/40 transition-colors duration-150 hover:bg-white/5 hover:text-white/80">
            {collapsed ? <ChevronsRight className="h-4 w-4" /> : <ChevronsLeft className="h-4 w-4" />}
          </button>}
          {isMobile && <button type="button" onClick={() => setMobileOpen(false)} className="rounded-md p-1.5 text-white/40"><X className="h-5 w-5" /></button>}
        </div>
      </div>
      ...
      <div className="shrink-0 border-t border-sidebar-border p-3">
        <div className={cn("flex items-center gap-3 rounded-md px-2 py-2", collapsed && !isMobile && "justify-center px-0")}>
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/20 text-caption font-semibold text-primary">{initials}</div>
          {(isMobile || !collapsed) && <div className="flex min-w-0 flex-1 items-center justify-between">
            ...
            <button type="button" onClick={signOut} className="rounded-md p-1.5 text-white/40 transition-colors duration-150 hover:bg-white/5 hover:text-white/80" title="התנתקות"><LogOut className="h-4 w-4" /></button>
          </div>}
        </div>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <aside className={cn("fixed inset-y-0 right-0 z-50 w-72 transform bg-sidebar shadow-lg transition-transform duration-300 md:hidden", mobileOpen ? "translate-x-0" : "translate-x-full")}>{content(true)}</aside>
    </>
  );
}
