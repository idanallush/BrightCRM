...
- **סטטוס משימה: 5 ערכים** (מעודכן 2026-05-25 לאחר QA — הותאם למצב בפועל ב-UI).
  ...
  **חוב טכני שנותר** (נמצא ב-/qa 2026-05-25, לא תוקן):
  - `lib/data.ts:8` מצהיר type union של 7 ערכים (כולל `אישור מנהל`,
    `בוטל`) — שינוי הסוג דורש בדיקה שאין רשומות עם הערכים הללו ב-DB.
  - `task-table.tsx:34` ו-`client-tasks-section.tsx:10` מתייחסים ל-`סגור`
    ו-`בוטל` כסטטוסי-סיום — צריך לרענן את הרשימות לאחר ניקוי הסוג.
